...
**Firmware:** v0.2.6 · **Updated:** 2026-07-15 · **Applies to:** Avalon Nano 3s + Avalon Q
...
|---|---|---|---|
| `version` | string | `"0.2.6"` | TNA-OS firmware version |
...
```
- **Power ON** — energise the hashboard and start mining (~15 s).
- **Power OFF** — stop mining and de-energise the hashboard. On the Q the rail is cut immediately
  (a safety action, independent of the mining loop). The control board, UI, network and telemetry
  stay fully online — only the hashboard goes off.
...
A reboot is **required** for boot-time settings such as `auto_power_on` to take effect.
