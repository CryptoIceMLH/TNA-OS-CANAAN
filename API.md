...
REST/JSON API for the **Avalon Nano 3s** (Kendryte K230D control SoC + 12× A3197S ASICs)
running TNA-OS-CANAAN. This is the integration contract for building monitoring dashboards,
control panels, fleet managers, and automation against a miner.
...
|---|---|---|---|
| `version` | string | `"0.9.53"` | TNA-OS firmware version |
...
|---|---|---|---|
| `power` | number | W | USB-C PD input power (INA226) |
...
| `coreVoltageActual` | int | mV | Live Vcore |
| `defaultCoreVoltage` | int | mV | Firmware default |
| `minVoltage` / `maxVoltage` | int | mV | 3100 / 3900 |
| `maxPower` | number | W | ~134 |
| `psuPdActive` | bool | | USB-C PD contract negotiated |
| `powerValid` | bool | | INA226 readings valid. `false` → `current`/`power`/`hashEfficiency` are not meaningful |
| `psuBypass` | bool | | PSU-bypass mode active (external DC injection instead of USB-C PD) |
...
| `fanRpm` | int | **Live tach RPM** (e.g. `960`) |
...
| `wifiRSSI` | int | WiFi signal strength (dBm; `0` on Ethernet) |
...
| `chipNonces30s` | int[] | Per-chip nonce counts (last 30 s) — relative chip health |
...
### 3.6 `POST /api/system/restart` and `POST /api/system/reboot`
Both perform a **full device reboot** (`/sbin/reboot`, ~500 ms delay) — ~60–90 s downtime.
```bash
curl -X POST http://<miner-ip>/api/system/reboot
```
Response: `{"ok":true,"msg":"rebooting"}`.

> `POST /api/system/power-cycle` exists but returns
> `{"ok":false,"err":"power-cycle not supported on Canaan Nano 3s (USB-C PD, no external PSU)"}` —
> the Nano 3s has no GPIO-switched PSU rail. Use `reboot`.

### 3.7 `POST /api/system/ping`
...
**Control-SoC (K230D):** `cpuLoad1m`, `cpuLoad5m`, `cpuLoad15m`, `cpuFreqMhz`,
`cpu0FreqMhz`, `cpu1FreqMhz`, `ramTotalMb`, `ramUsedMb`, `freeHeap`, `freeHeapInt`.
...
`dropped30s`, `uartPath`, `uartBaud`, `connector`, `resetGpio`, `domains`, `domainSize`,
`sensorFault`, `freqOverride`, `freqOverrideMhz`, `voltageActual`, `attempt`.

**LED (`led`):** `r`, `g`, `b`, `brightness`, `override` (all 0–255 / bool).
Writable via `PATCH /api/system` `{"led":{...}}`.
